...
import { 
  ...
  UploadCloud, AlertTriangle, CheckCircle, XCircle 
} from "lucide-react";
  ...
  useEffect(() => {
    const fetchShow = async () => {
      try {
        ...
        navigate("/bookings"); // Nếu lỗi thì đá về trang danh sách
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="bg-purple-50 p-5 rounded-xl border border-purple-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button 
                  onClick={() => toast.success("Đã gửi yêu cầu! (Tính năng đang phát triển)")}
                  className="bg-purple-600 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg hover:bg-purple-700 transition active:scale-95 whitespace-nowrap"
                >
                  ✋ Đăng ký ngay
                </button>
              </div>

              {/* Danh sách thành viên (Demo) */}
              <div className="mt-6">
                <p className="text-xs font-bold text-gray-400 uppercase mb-3">Đội hình hiện tại</p>
                <div className="space-y-3">
                  <div className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
                    <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center font-bold text-blue-700 text-xs">A</div>
                    <span className="font-bold text-gray-700 text-sm">Admin (Bạn)</span>
                    <span className="ml-auto text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded border">Chưa xếp vai trò</span>
                  </div>
                  {/* Chỗ trống */}
                  <div className="flex items-center gap-3 p-3 border-2 border-dashed border-gray-200 rounded-xl text-gray-400">
                    ...
                    <span className="text-sm italic">Đang chờ thêm thành viên...</span>
                  </div>
                </div>
              </div>
            </div>
