...
const BookingDetail = () => {
  ...
  // 2. Xử lý Đăng ký / Hủy tham gia (API thật)
  const handleJoin = async () => {
    try {
      ...
      console.error(err);
      // Lỗi 404 nghĩa là chưa Update Backend
      toast.error(err.response?.status === 404 ? "Lỗi: Server chưa có tính năng này (Hãy push code Backend)" : "Lỗi thao tác");
    }
  };
  ...
  // Kiểm tra xem User hiện tại đã có trong danh sách chưa
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              <div className="bg-purple-50 p-5 rounded-xl border border-purple-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                {/* Nút bấm Đăng ký / Hủy - Đổi màu và chữ theo trạng thái */}
                <button 
                  onClick={handleJoin}
                ...
                <div className="space-y-3">
                  {show.participants && show.participants.length > 0 ? (
                    show.participants.map((p) => (
                      <div key={p._id || Math.random()} className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl shadow-sm hover:border-purple-200 transition">
                        ...
                        <span className="font-bold text-gray-700 text-sm">{p.user?.fullName || "Thành viên cũ"}</span>
                        
                        {/* Đánh dấu nếu là chính mình */}
                        {p.user?._id === user?._id && (
                          <span className="ml-auto text-xs bg-green-50 text-green-700 px-2 py-1 rounded border border-green-100 font-bold">
                            Bạn
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-3 p-3 border-2 border-dashed border-gray-200 rounded-xl text-gray-400 bg-gray-50">
                       <span className="text-sm italic mx-auto">Chưa có ai đăng ký... Hãy là người đầu tiên!</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Box List Nhạc (Giữ nguyên giao diện chờ phát triển sau) */}
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
