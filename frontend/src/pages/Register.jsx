...
const Register = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-[#f8fafc] font-sans">
      ...
      <div className="absolute top-[-10%] right-[-10%] w-96 h-96 bg-blue-200/40 rounded-full mix-blend-multiply filter blur-[100px] opacity-70 animate-blob"></div>
      <div className="absolute bottom-[-10%] left-[-10%] w-96 h-96 bg-violet-200/40 rounded-full mix-blend-multiply filter blur-[100px] opacity-70 animate-blob animation-delay-2000"></div>

      {/* Box Đăng ký */}
      <div className="relative w-full max-w-lg bg-white/80 backdrop-blur-xl border border-white/60 rounded-[32px] p-8 sm:p-10 z-10 shadow-2xl shadow-slate-200/50 animate-fade-in my-10">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-tr from-blue-600 to-violet-600 text-white shadow-lg shadow-blue-500/20 mb-4">
            <UserPlus size={28} />
          </div>
          <h2 className="text-3xl font-black text-slate-800 tracking-tight">ĐĂNG KÝ THÀNH VIÊN</h2>
        ...
        <form onSubmit={handleInitialSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="relative group">
              <User className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-blue-600 transition" size={18} />
              <input name="fullName" placeholder="Họ và tên" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 ring-blue-500/30 focus:border-blue-600 text-slate-800 transition text-sm font-medium placeholder-slate-400" onChange={handleChange} required />
            </div>
            <div className="relative group">
              <Music className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-blue-600 transition" size={18} />
              <input name="username" placeholder="Tên đăng nhập" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 ring-blue-500/30 focus:border-blue-600 text-slate-800 transition text-sm font-medium placeholder-slate-400" onChange={handleChange} required />
            </div>
          </div>

          <div className="relative group">
            <Mail className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-blue-600 transition" size={18} />
            <input name="email" type="email" placeholder="Địa chỉ Email" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 ring-blue-500/30 focus:border-blue-600 text-slate-800 transition text-sm font-medium placeholder-slate-400" onChange={handleChange} required />
          </div>

          <div className="relative group">
            <Phone className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-blue-600 transition" size={18} />
            <input name="phone" placeholder="Số điện thoại" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 ring-blue-500/30 focus:border-blue-600 text-slate-800 transition text-sm font-medium placeholder-slate-400" onChange={handleChange} required />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="relative group">
              <Lock className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-blue-600 transition" size={18} />
              <input name="password" type="password" placeholder="Mật khẩu" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 ring-blue-500/30 focus:border-blue-600 text-slate-800 transition text-sm font-medium placeholder-slate-400" onChange={handleChange} required />
            </div>
            <div className="relative group">
              <Lock className="absolute left-4 top-3.5 text-slate-400 group-focus-within:text-blue-600 transition" size={18} />
              <input name="confirmPassword" type="password" placeholder="Nhập lại" className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200/60 rounded-xl focus:outline-none focus:ring-2 ring-blue-500/30 focus:border-blue-600 text-slate-800 transition text-sm font-medium placeholder-slate-400" onChange={handleChange} required />
            </div>
          </div>

          <button className="w-full bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-700 hover:to-violet-700 text-white font-black py-4 rounded-xl shadow-lg shadow-blue-500/20 transform active:scale-95 transition-all flex items-center justify-center gap-2 mt-4 tracking-widest text-xs">
            TIẾP TỤC <ArrowRight size={18} />
          </button>
        </form>

        <p className="text-center mt-8 text-slate-500 text-sm font-medium">
          Đã có tài khoản? <Link to="/login" className="text-blue-600 font-bold hover:text-blue-700 transition underline underline-offset-4">Đăng nhập ngay</Link>
        </p>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white border border-slate-200 rounded-[32px] max-w-xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl animate-slide-up">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h3 className="font-black text-xl text-slate-850 flex items-center gap-2">
                <FileText size={24} className="text-blue-600" /> Quy Định Nội Bộ
              </h3>
            ...
            <div ref={termsBodyRef} onScroll={handleScroll} className="p-8 overflow-y-auto space-y-8 text-slate-600 text-sm leading-relaxed scroll-smooth">
              ...
              <div className="space-y-3">
                <h4 className="font-bold text-slate-850 flex items-center gap-2 text-base">
                  <Shield size={18} className="text-blue-600" /> 1. Bảo mật Hệ thống
                </h4>
              ...
              <div className="space-y-3">
                <h4 className="font-bold text-slate-850 flex items-center gap-2 text-base">
                  <Music size={18} className="text-fuchsia-600" /> 3. Chuẩn bị Chuyên môn
                </h4>
              ...
              <div className="space-y-3">
                <h4 className="font-bold text-slate-850 flex items-center gap-2 text-base">
                  <DollarSign size={18} className="text-emerald-600" /> 4. Phân chia Cát-xê & Quỹ nhóm
                </h4>
                <ul className="space-y-2 list-none">
                  <li className="flex gap-2">
              ...
              <div className="space-y-3">
                <h4 className="font-bold text-slate-850 flex items-center gap-2 text-base">
                  <User size={18} className="text-violet-600" /> 5. Tác phong & Trang phục
                </h4>
              ...
              <div className="p-5 bg-gradient-to-r from-blue-500/5 to-violet-500/5 border border-blue-500/10 rounded-2xl text-slate-500 text-xs italic text-center leading-relaxed">
