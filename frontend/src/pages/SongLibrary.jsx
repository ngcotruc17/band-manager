...
import { Music, UploadCloud, Trash2, Search, FileText, PlayCircle, Plus, X, Headphones, Loader } from "lucide-react";
...
const SongLibrary = () => {
  ...
  // Helper function to resolve media URL base
    ...
    <div className="min-h-screen bg-[#f8fafc] p-4 md:p-8 pt-8 animate-fade-in transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-10">
          <div>
            <h1 className="text-3xl font-black text-slate-800 flex items-center gap-3">
              <span className="bg-gradient-to-tr from-blue-650 to-indigo-600 text-white p-2.5 rounded-2xl shadow-lg shadow-blue-500/10">
                <Music size={28} />
              </span>
              KHO BEAT & SHEET
            </h1>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-80">
              ...
              <input
                className="w-full pl-12 pr-4 py-3.5 rounded-2xl border border-slate-200 bg-white shadow-sm focus:ring-2 ring-blue-500 outline-none text-slate-800 font-semibold transition"
                placeholder="Tìm bài hát, tone, điệu..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <button onClick={() => setShowModal(true)} className="bg-slate-900 hover:bg-black text-white px-6 py-3.5 rounded-2xl font-bold shadow-xl transition flex items-center gap-2 whitespace-nowrap active:scale-[0.98] text-xs uppercase tracking-wider">
        ...
        {fetchLoading ? (
          ...
          <div className="text-center py-20 glass rounded-[40px] border-dashed border-slate-200">
            <div className="bg-slate-100/50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200">
              <Music size={40} className="text-slate-350" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredSongs.map((song) => (
              <div key={song._id} className="glass rounded-[32px] overflow-hidden group flex flex-col card-hover">
                <div className="p-6 flex-1">
                  <div className="flex justify-between items-start mb-4">
                    <div className="bg-blue-50 text-blue-600 p-3 rounded-2xl border border-blue-100/20">
                      <Headphones size={24} />
                    </div>
                    {user?.role === "admin" && (
                      <button onClick={() => handleDelete(song._id)} className="text-slate-300 hover:text-rose-500 transition p-2 rounded-full hover:bg-rose-50 border border-transparent">
                        <Trash2 size={18} />
                      </button>
                    )}
                  </div>
                  <h3 className="font-black text-slate-850 text-lg line-clamp-2 leading-tight mb-2 group-hover:text-blue-600 transition tracking-tight">
                ...
                <div className="bg-slate-50/50 p-5 border-t border-slate-100 flex flex-col gap-3">
                  ...
                  {song.beatUrl ? (
                    <button 
                      ...
                      className="flex justify-between items-center bg-slate-900 text-white p-3 rounded-xl shadow-md hover:bg-black transition group/play"
                    >
                      ...
                      {currentTrack?.url?.includes(song.beatUrl) && isPlaying && (
                         <div className="flex items-end gap-0.5 h-3.5 w-6 px-1">
                           <div className="w-[3px] bg-blue-400 h-full origin-bottom sound-bar-1 rounded-sm"></div>
                           <div className="w-[3px] bg-blue-400 h-full origin-bottom sound-bar-2 rounded-sm"></div>
                           <div className="w-[3px] bg-blue-400 h-full origin-bottom sound-bar-3 rounded-sm"></div>
                           <div className="w-[3px] bg-blue-400 h-full origin-bottom sound-bar-4 rounded-sm"></div>
                         </div>
                      )}
                    </button>
                  ) : (
      ...
      {showModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-[40px] w-full max-w-lg shadow-2xl overflow-hidden border border-slate-200 animate-slide-up">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              ...
              <button onClick={() => setShowModal(false)} className="text-slate-400 hover:text-rose-500 bg-white p-2 rounded-full transition shadow-sm border border-slate-100">
                <X size={24} />
              </button>
            </div>
            ...
            <form onSubmit={handleUpload} className="p-8 space-y-6">
              ...
              <div>
                ...
                <input
                  className="w-full p-4 border border-slate-200 rounded-2xl focus:ring-2 ring-blue-500 outline-none bg-slate-50 font-medium text-slate-750 transition"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="relative group cursor-pointer">
                  <div className={`p-5 border-2 border-dashed rounded-3xl transition text-center ${files.sheet ? 'bg-rose-50 border-rose-200 text-rose-600' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
                ...
                <div className="relative group cursor-pointer">
                  <div className={`p-5 border-2 border-dashed rounded-3xl transition text-center ${files.beat ? 'bg-blue-50 border-blue-200 text-blue-600' : 'bg-slate-50 border-slate-200 text-slate-400'}`}>
      ...
      {/* Bounce-animated CSS sound wave style rules */}
      <style>{`
        @keyframes bounce-1 {
        ...
        .sound-bar-1 { animation: bounce-1 0.75s ease-in-out infinite; }
        .sound-bar-2 { animation: bounce-2 0.65s ease-in-out infinite; }
        .sound-bar-3 { animation: bounce-3 0.85s ease-in-out infinite; }
        .sound-bar-4 { animation: bounce-4 0.55s ease-in-out infinite; }
      `}</style>
    </div>
  );
};
