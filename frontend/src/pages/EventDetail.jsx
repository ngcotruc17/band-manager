...
const EventDetail = () => {
  ...
  const loadData = async () => {
    try {
      const res = await axios.get(`http://localhost:5000/api/events/${id}`, getHeaders());
      setData(res.data);
      setEditEventData({
  ...
  const handleJoin = async () => {
    try {
      await axios.post(`http://localhost:5000/api/events/${id}/join`, {}, getHeaders());
  ...
  const handleTogglePerformer = async (userId) => {
    try {
      await axios.put(`http://localhost:5000/api/events/${id}/performer`, { userId }, getHeaders());
      loadData();
  ...
  const handleAddSong = async (e) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/events/${id}/songs`, postData, {
  ...
  const handleAddFromLib = async (libSongId) => {
    try {
      await axios.post(`http://localhost:5000/api/events/${id}/songs/from-library`, { librarySongId: libSongId }, getHeaders());
  ...
  const handleUpdateEvent = async () => {
    try {
      await axios.put(`http://localhost:5000/api/events/${id}`, editEventData, getHeaders());
      alert("✅ Đã cập nhật!");
      setIsEditing(false);
      loadData();
  ...
  const handleDeleteSong = async (songId) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/events/songs/${songId}`, getHeaders());
      loadData();
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/events/${id}`, getHeaders());
      alert("🗑️ Đã xóa show!");
      navigate('/dashboard');
                ...
                <div key={song._id} className="bg-white p-4 rounded shadow-sm border border-gray-100 flex justify-between items-start hover:shadow-md transition">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-3 mt-2">
                      {song.sheetUrl && <a href={`http://localhost:5000/${song.sheetUrl}`} target="_blank" className="text-red-600 bg-red-50 px-3 py-1 rounded border border-red-200 text-sm font-bold hover:bg-red-100 transition">📄 Sheet</a>}
                      {song.beatUrl && <audio controls src={`http://localhost:5000/${song.beatUrl}`} className="h-8 w-64" />}
                    </div>
                  </div>
                  {user?.role === 'admin' && (
