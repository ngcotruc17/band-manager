...
const fmtMoney = (val: number) => new Intl.NumberFormat('vi-VN').format(val || 0) + 'đ';

// 1. Gửi mail cho TOÀN BỘ thành viên khi có Show mới
export const sendNewShowEmail = async (show: any): Promise<void> => {
  try {
    const users = await User.find({ email: { $exists: true, $ne: null }, isApproved: true });
    const emails = users.map(u => u.email).filter(email => email && email.includes('@')) as string[];
    if (emails.length === 0) return;

    // Link web
    const webUrl = `http://localhost:3000/bookings/${show._id}`;
...
export const sendApproveEmail = async (userEmail: string, showTitle: string, userName: string): Promise<void> => {
  try {
...
export const sendNewRehearsalEmail = async (rehearsal: any): Promise<void> => {
  try {
    const users = await User.find({ email: { $exists: true, $ne: null }, isApproved: true });
    const emails = users.map(u => u.email).filter(email => email && email.includes('@')) as string[];
    if (emails.length === 0) return;
...
export const sendSalarySplitEmail = async (show: any): Promise<void> => {
  try {
...
export const sendCustomAdminEmail = async (subject: string, contentHtml: string, recipientEmails: string[]): Promise<void> => {
  try {
